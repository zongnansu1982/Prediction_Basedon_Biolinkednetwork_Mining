 ...
 * @author Alex Black
 */
public class Deepwalk_enhenced<V, E> extends GraphVectorsImpl<V, E> {
    public static final int STATUS_UPDATE_FREQUENCY = 1000;
    private Logger log = LoggerFactory.getLogger(Deepwalk_enhenced.class);
    ...
    public Deepwalk_enhenced(int numberOfThreads) {
    	nThreads=numberOfThreads;
    }
    public Deepwalk_enhenced() {
	...
	public void setNumberOfWalk(int numberOfWalk) {
    ...
    public void fit(GraphWalkIterator<V> iterator) {
        ...
        for (int perwalk = 0; perwalk < this.numberOfWalk; perwalk++) {
    ...
    public static class Builder<V, E> {
        ...
        public Builder<V, E> numberOfWalk(int numberofWalk) {
            this.numberofWalk = numberofWalk;
            return this;
        }
        ...
        public Deepwalk_enhenced<V, E> build() {
        	Deepwalk_enhenced<V, E> dw = new Deepwalk_enhenced<>();
            dw.vectorSize = vectorSize;
            dw.windowSize = windowSize;
            dw.learningRate = learningRate;
            dw.seed = seed;
            dw.numberOfWalk=numberofWalk;
            return dw;
        }
        
        public Deepwalk_enhenced<V, E> build(int numberOfThread) {
        	Deepwalk_enhenced<V, E> dw = new Deepwalk_enhenced<>(numberOfThread);
            dw.vectorSize = vectorSize;
            dw.windowSize = windowSize;
            dw.learningRate = learningRate;
            dw.seed = seed;
    ...
    private class LearningCallable implements Callable<Void> {
        ...
        LearningCallable(GraphWalkIterator<V> iter){
        	iterator=iter;
        }
        @Override
